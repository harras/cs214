...
void* mymalloc(int memRequest, char* filename, int line){
    ...
    if((unsigned short) myblock[0]==0){
        unsigned short firstblock=4;
        myblock[0]=firstblock;
        unsigned short blankmem=0;
        myblock[2]=blankmem;
    }
   
    // if the request exceeds maximum available memory, does not try to allocate
    if((memReqShort+4)>5000){
    ...
    while(myblock[currindex]!=0 && ((unsigned short) myblock[currindex]-(currindex+4+memReqShort))<(memReqShort+4) ){
        currindex=(unsigned short) myblock[currindex];
    }
    ...
    if(currindex+4+(memReqShort+4)>5000){
    ...
    myblock[nextindex+2]=(unsigned short) memReqShort;
    myblock[currindex]=nextindex;
...
void myfree(void* memptr, char* filename, int line){
    ...
    while(freeIndex>(unsigned short) myblock[index] && freeIndex!=index+4 && (unsigned short) myblock[prev]!=0){
        prev=index;
        index=myblock[index];      
    }
    ...
    if(freeIndex==index+4){
        ...
        myblock[prev]=(unsigned short) myblock[index];
